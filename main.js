...
;// ./src/js/components/MessageService.js
...
class MessageService {
  ...
  fetchMessages() {
    return source => source.pipe(switchMap(() => ajax.getJSON(this.apiUrl)), catchError(() => of({
      messages: []
    })));
  }
  processMessages() {
...
class MessageList {
  ...
  animateNewMessages(count) {
    ...
    Array.from(newRows).slice(0, count).forEach(row => {
...
const messageService = new MessageService("https://js-3-hw-11-rxjs-polling-backend.onrender.com/messages/unread");
const API_URL = '';
const messageList = new MessageList(document.getElementById("message-container"));

// Настройка потока данных
interval(POLLING_INTERVAL).pipe(messageService.fetchMessages(),
